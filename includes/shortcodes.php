...
class SupTic_ShortcodeManager {
	...
	function do_shortcode_tag( $m ) {
		...
		if ( is_array( $attr ) ) {
			...
			if ( SUPTIC_USE_PIPE ) {
				$pipes = new SupTic_Pipes( $scanned_tag['raw_values'] );
		...
		if ( $this->exec ) {
			$scanned_tag = apply_filters( 'suptic_form_tag', $scanned_tag );
	...
	function shortcode_parse_atts( $text ) {
		...
		if ( preg_match( $pattern, $text, $match ) ) {
			...
			if ( ! empty( $match[2] ) ) {
				...
				$atts['values'] = suptic_strip_quote_deep( $matched_values[0] );
			}
		} else {
			$atts = $text;
		}
...
$suptic_shortcode_manager = new SupTic_ShortcodeManager();

function suptic_add_shortcode( $tag, $func, $has_name = false ) {
	global $suptic_shortcode_manager;

	return $suptic_shortcode_manager->add_shortcode( $tag, $func, $has_name );
}

function suptic_remove_shortcode( $tag ) {
	global $suptic_shortcode_manager;

	return $suptic_shortcode_manager->remove_shortcode( $tag );
}

function suptic_do_shortcode( $content ) {
	global $suptic_shortcode_manager;

	return $suptic_shortcode_manager->do_shortcode( $content );
}

function suptic_get_shortcode_regex() {
	global $suptic_shortcode_manager;

	return $suptic_shortcode_manager->get_shortcode_regex();
}
